...
int main(int argc, char *argv[])
{
    ...
    char buffer[10] = ""; // buffer stockant le message
    ...
    do
    {
        /* code */
        draw_pendu(2, 5);
        // printf("Entrer un message a envoyer ('.' pour arreter): ");
        scanf(" %c", &buffer[0]);
        buffer[1] = '\0';
        // Envoi du message
        ...
        default: /* envoi de n octets */
            printf("Message %c envoyé! (%d octets)\n\n", buffer[0], nb);
            lus = recv(descripteurSocket, reponse, sizeof(reponse) - 1, 0);
            if (lus > 0)
            {
                reponse[lus] = '\0'; // s'assurer que c'est une chaîne C terminée par \0
