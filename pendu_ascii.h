...
// Fonction pour obtenir le dessin sous forme de chaîne
static void get_pendu_ascii(int nb_fautes, char *buffer, size_t size) {
    if (nb_fautes < 0) nb_fautes = 0;
    if (nb_fautes >= NB_DESSINS) nb_fautes = NB_DESSINS - 1;
    
    buffer[0] = '\0';
    for (int i = 0; i < LIGNES_PENDU; i++) {
        strncat(buffer, pendu[nb_fautes][i], size - strlen(buffer) - 1);
